...
class TC_10_BackupCompatibility(qubesadmin.tests.backup.BackupTestCase):
    ...
    def create_v4_files(self):
        ...
        for vm in ('test-work', 'test-d8test', 'test-proxy',
            ...
            os.mkdir(self.fullpath(f'appvms/{vm}'))
            self.create_whitelisted_appmenus(self.fullpath(
                f'appvms/{vm}/whitelisted-appmenus.list'))
            self.create_private_img(self.fullpath(
                f'appvms/{vm}/private.img'))
        ...
        for vm in ('test-standalonevm', 'test-hvm'):
            ...
            self.create_private_img(self.fullpath(
                f'appvms/{vm}/private.img'))
            self.create_sparse(
                self.fullpath(f'appvms/{vm}/root.img'), 10*2**30)
            self.fill_image(self.fullpath(f'appvms/{vm}/root.img'),
                1024*1024, True,
                signature=f'{vm}/root'.encode())
    ...
    def handle_v3_file(self, f_name, subdir, stream, compressed=True,
        ...
        tar_cmdline = ["tar", "-Pc", '--sparse',
               ...
               '--xform', f's:^{os.path.basename(f_name)}:{subdir}\\0:',
               os.path.basename(f_name)
               ]
        if compressed:
            tar_cmdline.insert(-1, f"--use-compress-program=gzip")
        ...
        for part in sorted(os.listdir(stage1_dir)):
            if not re.match(
                    rf"^{os.path.basename(f_name)}.[0-9][0-9][0-9]$",
                    part):
                continue
    ...
    def handle_v4_file(self, f_name, subdir, stream, compressed="gzip"):
        ...
        if compressed and isinstance(compressed, str):
            tar_cmdline.insert(-1, f"--use-compress-program={compressed}")
        elif compressed:
            tar_cmdline.insert(-1, f"--use-compress-program=gzip")
        with subprocess.Popen(tar_cmdline, stdout=subprocess.PIPE) as tar:
            data = tar.stdout
            ...
            with open(self.fullpath("qubes.xml"), "wb") \
                ...
                if encrypted:
                    for vmname, subdir in MANGLED_SUBDIRS_R2.items():
                        qubesxml = re.sub(rf"[a-z-]*/{vmname}".encode(),
    ...
    def setup_expected_calls(self, parsed_qubes_xml, templates_map=None):
        ...
        for name, vm in parsed_qubes_xml['domains'].items():
            ...
            extra_vm_list_lines.append(
                f"{name} class={vm['klass']} state=Halted\n".encode())
            if vm['backup_path']:
                self.app.expected_calls[
            ...
            for bus, devices in vm['devices'].items():
                for (backend_domain, port_id), _ in devices.items():
                    ...
                    self.app.expected_calls[
                        (name, f'admin.vm.device.{bus}.Assign',
                        f'{backend_domain}+{port_id}+_',
